...
export class PokeAll extends LitElement {
    ...
    async extractOffsetAndUpdatePage(mode: previous_next.NEXT | previous_next.PREVIOUS) {
        let url = null
        mode === previous_next.NEXT ? url = new URL(this.nextPage) : url = new URL(this.previousPage)
        const urlParams = new URLSearchParams(url!.search)
        this.offset = urlParams.get('offset')
        this.pokemonall = await pokemonApi.getAllPokemon(this.offset, pagination.LIMIT_OFFSET);
