...
export class PokeSearch extends LitElement {
    ...
    getCurrentResearch(existing: any, current: any) {
        try {
            return existing.results.filter(
                (pokemon: any) => pokemon.name == current
            )
        } catch (error) {
                ...
                ${this.currentCache.map(
                    (pokemonName: string) =>
                        html` <li
                            ...
                            @click="${() =>
                                this.submitResearchByCache(pokemonName)}"
                        >
    ...
    submitResearchByCache(pokemoname: any) {
        cache
            ...
            .then(async (cache) => {
                if (cache) {
                    console.info(
                        'existing cache from last research',
                        pokemoname
                    )
    ...
    render() {
        return html`
            <div
                class="pokesearch-body  ${this.toggleWrapClass
                    ? 'toggleWrap'
                    : ''} "
            >
                <div class="container">
