...
export class PokeSearch extends LitElement {
  ...
  getPokeRandom() {
    const data = html` <poke-random><poke-random /> </poke-random> `
    return data
  }
  ...
  async searchPokemon(e: any) {
    ...
    if (existingCache) {
      ...
      this.encounters = await pokemonApi.getPokemonEncounters(
        neededPokemon.location_area_encounters
      )
      this.showLoader = false

      storeEventConst.dispatch({
        type: eventType.IS_SEARCH_ACTIVATED,
        data: true,
      })
    }
  ...
  async displayLastResearch() {
    ...
    if (!this.notShowMoreCachePokemon) {
      ...
      currentCacheOtherResult = this.currentCache.slice(
        NBR_SHOW_POKEMON,
        this.currentCache.length
      )
    } else {
    ...
    return html`
      <ul class="pokesearch-lastresearch">
        ...
        <li
          tabindex="0"
          class="pokesearch-showmore"
          style="color: white"
          ?hidden=${this.notShowMoreCachePokemon &&
          !this.notShowMoreCachePokemon &&
          !currentCacheFormatted.length}
          @click="${() =>
            this.showMoreOrLess(currentCacheOtherResult, ShowMoreOrLess.MORE)}">
          Show more
        </li>
        <li
          tabindex="0"
          class="pokesearch-showless"
          style="color: white"
          ?hidden=${!this.notShowMoreCachePokemon}
          @click="${() =>
            this.showMoreOrLess(this.currentCache, ShowMoreOrLess.LESS)}">
          Show less
        </li>
      </ul>
    `
    ...
    cache
      ...
      .then(async (cache) => {
        if (cache) {
          ...
          const encounters = await pokemonApi.getPokemonEncounters(
